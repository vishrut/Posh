  ...
  <head>
	...
	<body>
		...
		<div class="container" style="width: auto; padding: 20px 0px 0px 0px;">
			...
			$offers = DB::query("SELECT * FROM $tbl_name where seller=%s",$_SESSION['username']);
			$counter = 0;
			...
			foreach($offers as $offer){
				...
				if($item['owner']==$_SESSION['username']){	
					if($counter!=0 && $counter%4==0){
						...
						if($offer['ssr']=='rent')	
						echo '<p><a href="viewofferrent.php?offerid='.$offer['offerid'].'"> <button class="btn btn-primary">View Offer</button></a>'; 
						if($offer['ssr']=='swap')	
						echo '<p><a href="viewofferswap.php?offerid='.$offer['offerid'].'"> <button class="btn btn-primary">View Offer</button></a>'; 
						if($offer['ssr']=='sell')	
						echo '<p><a href="viewofferbuy.php?offerid='.$offer['offerid'].'"> <button class="btn btn-primary">View Offer</button></a>';
